...
int main()
{
    ...
    cin>>isHigher;
    int totalSeekTime;
    if(isHigher)totalSeekTime=(maxUserRequest-head)+(maxUserRequest-minUserRequest);
    else totalSeekTime=(head-minUserRequest)+(maxUserRequest-minUserRequest);
    cout<<"Total seek time using LOOK disk scheduling:"<<totalSeekTime<<endl;
    cout<<"Average seek time using LOOK disk scheduling:"<<totalSeekTime*1.0/n<<endl;
    return 0;
}