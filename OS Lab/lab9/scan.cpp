...
int main()
{
    ...
    cin>>isHigher;
    int totalSeekTime;
    if(isHigher)totalSeekTime=(maxRequest-head)+(maxRequest-minUserRequest);
    else totalSeekTime=(head-minRequest)+(maxUserRequest-minRequest);
    cout<<"Total seek time using SCAN disk scheduling:"<<totalSeekTime<<endl;
    cout<<"Average seek time using SCAN disk scheduling:"<<totalSeekTime*1.0/n<<endl;
    return 0;
}